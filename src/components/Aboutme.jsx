import { useEffect, useState, useContext, useRef } from 'react';
import { ScrollTrigger } from 'gsap/dist/ScrollTrigger';
import gsap from 'gsap';
import SplitType from 'split-type';
import Lenis from '@studio-freight/lenis';

function Aboutme() {
    const [rendered, setRendered] = useState(false);
 

    useEffect(() => {

         // hightlight nav mobile menu btn
      document.querySelectorAll('.mobnav-btn').forEach(el => el.classList.remove('mobmenu-btn-active'));
      document.querySelector('.mobnav-aboutme').classList.add('mobmenu-btn-active');
      document.querySelector('.csmright_dmore_wrp').style.opacity = '0';


        if(!rendered) {
       
               
            setRendered(true);

            gsap.registerPlugin(ScrollTrigger)

            const splitTypes = document.querySelectorAll('.reveal-type')
    
            splitTypes.forEach((char,i) => {
    
                const bg = char.dataset.bgColor
                const fg = char.dataset.fgColor
    
                const text = new SplitType(char, { types: 'char'})
    
                gsap.fromTo(text.chars, 
                    {
                        color: bg,
                    },
                    {
                        color: fg,
                        duration: 0.5,
                        stagger: 0.01,
                        scrollTrigger: {
                            trigger: char,
                            start: 'top 80%',
                            end: 'top 20%',
                            scrub: true,
                            markers: false,
                            toggleActions: 'play play reverse reverse'
                        }
                })
            })
            
            const fillBarSpan = document.querySelector('.ccsml-fill-scr span');

            const totalHeight = document.documentElement.scrollHeight - window.innerHeight;
            
            function updateFillBarWidth() {
                const scrollPosition = window.scrollY;
                const fillWidth = (scrollPosition / totalHeight) * 100;
                fillBarSpan.style.width = `${fillWidth}%`;
              }
        
            let lenis = new Lenis()
    
            lenis.on('scroll', updateFillBarWidth);
    
            function raf(time) {
            lenis.raf(time);
            updateFillBarWidth();
            requestAnimationFrame(raf);
            }
    
            requestAnimationFrame(raf);
            

            document.querySelector('.headernavwrp-aboutme').classList.remove('d-none');
            setTimeout(() => {

                // hide more
                document.querySelector('.headernavwrp-aboutme').style.opacity = '1';
               document.querySelector('.headernavwrp-aboutme').classList.remove('disabled-nav');

            }, 1200);
         
                return () => {
                    lenis.destroy();
                }
        }
    },[])


    return (
        <div className='aboutme_container'>
            <section className='aboutme_section aboutme_section_one'>
                  <div className='aboutmesecone_wrp'>
                        <h1>FRONT-END <span>DEVELOPER</span></h1>  
                        <h3>BUCHAREST</h3>
                        <p>Scroll down for more</p>
                  </div>
            </section>

            <section className='aboutme_section aboutme_section_two'>
                <p className="p_style revpass-type">I'm <span>a</span> Front-End Developer with a <span>passion for</span> turning ideas into interactive and user-friendly <span>digital</span> experiences.</p>
                <p className="p_style revpass-type">My focus is on <span>creative development</span> and crafting interfaces that bring <span>websites to life</span> effectively.</p>
            </section>

            <section className='aboutme_section aboutme_section_three'>
                <p className="aboutme_p_title">
                    Experience
                </p>
                <div className="aboutme_sec_exp_wrp aboume_secexp">
                     <ul>
                        <li>
                            <div className='abme_years abm_ul_li_div'>2022 - present</div>
                            <div className='abme_role abm_ul_li_div'>Front-End Developer</div>
                            <div className='abme_type abm_ul_li_div'>Remote</div>
                        </li>
                        <li>
                            <div className='abme_years abm_ul_li_div'>2019 - present</div>
                            <div className='abme_role abm_ul_li_div'>Front-End Developer</div>
                            <div className='abme_type abm_ul_li_div'>Portfolio</div>
                        </li>
                     </ul>
                </div>

                <p className="aboutme_p_title mt-5">
                    Working with
                </p>

                <div className="aboutme_sec_exp_wrp aboutmesec_exp_workingwith">
                     <ul>
                        <li>
                            <span>HTML5 / SCSS</span>
                            <span>JAVASCRIPT</span>
                            <span>Bootstrap 5 / MUI</span>
                            <span>Webpack / Vite</span>
                        </li>
                        <li>
                            <span>React js / Hooks</span>
                            <span>Redux / Context API</span>
                            <span>Laravel</span>
                            <span>Git</span>
                        </li>
                        <li>
                            <span>Firebase</span>
                            <span>FaunaDB</span>
                            <span>MySql</span>
                        </li>
                     </ul>
                </div>

                <p className="aboutme_p_title mt-5">
                    Familiar with
                </p>

                <div className="aboutme_sec_exp_wrp aboutmesec_exp_workingwith">
                     <ul>
                        <li>
                            <span>Three.js</span>
                            <span>Wordpress</span>
                            <span>Photoshop</span>

                        </li>

                     </ul>
                </div>

                
            </section>

            <section className='aboutme_section aboutme_section_contact'>
                <div className='aboutme_row'>
                    <p className="reveal-type p_style" data-bg-color="transparent" data-fg-color="white">SAY</p>
                    <p className="reveal-type p_style p_style_two" data-bg-color="transparent" data-fg-color="white">HELLO</p>

                    <p className='mailto_contact_p mt-4'><a className='mailto_contact' href='mailto:stanionut0122@gmail.com'>stanionut0122@gmail.com</a></p>

                    <div className='aboutme_social'>
                        <a href='https://github.com/SIonut0122?tab=repositories' target='_blank'>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-github" viewBox="0 0 16 16">
                                <path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.012 8.012 0 0 0 16 8c0-4.42-3.58-8-8-8z"/>
                            </svg>
                        </a>
                        <a href='https://www.linkedin.com/in/stan-ionut-1193a0159/' target='_blank'>
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-linkedin" viewBox="0 0 16 16">
                             <path d="M0 1.146C0 .513.526 0 1.175 0h13.65C15.474 0 16 .513 16 1.146v13.708c0 .633-.526 1.146-1.175 1.146H1.175C.526 16 0 15.487 0 14.854V1.146zm4.943 12.248V6.169H2.542v7.225h2.401zm-1.2-8.212c.837 0 1.358-.554 1.358-1.248-.015-.709-.52-1.248-1.342-1.248-.822 0-1.359.54-1.359 1.248 0 .694.521 1.248 1.327 1.248h.016zm4.908 8.212V9.359c0-.216.016-.432.08-.586.173-.431.568-.878 1.232-.878.869 0 1.216.662 1.216 1.634v3.865h2.401V9.25c0-2.22-1.184-3.252-2.764-3.252-1.274 0-1.845.7-2.165 1.193v.025h-.016a5.54 5.54 0 0 1 .016-.025V6.169h-2.4c.03.678 0 7.225 0 7.225h2.4z"/>
                            </svg>
                        </a>
                    </div>
                </div>

                <div className='aboutme_row'>
                <div className='aboutme_credits'>
                    <p>©  2023 — Ionut Stan</p>
                </div>
                </div>
            </section>


        </div>
    )
}

export default Aboutme;